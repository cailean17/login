...
def home():
    if request.method == 'GET':
        if 'login' not in session:
            ...
            y = mongo.db.postinfo.find().sort('time',-1).limit(5)
    ...
    if request.method == "POST":
        ...
        mongo.db.postinfo.insert_one(y)
...
def register():
    ...
    if request.method == 'POST':
        ...
        print(y)
        x = mongo.db.Login2.find_one({'email': y['email']})
        if x is not None:
            flash('Account with E-Mail already created.')
            return redirect('/login')
        mongo.db.Login2.insert_one(y)
...
def login():
    ...
    if request.method == 'POST':
        ...
        y = mongo.db.Login2.find_one({'email': email, 'password': password})
        if y is None:
